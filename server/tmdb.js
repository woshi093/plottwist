...
async function buildCatalogueFromTMDB(count = 14) {
  if (!TMDB_KEY) {
    throw new Error("TMDB_API_KEY is not set");
  }

  // Step 1: get a page of popular, reasonably well-known titles.
  const discover = await fetchJson(
    `${BASE}/discover/movie?sort_by=popularity.desc&vote_count.gte=200&include_adult=false&page=1`
  );
  const candidates = (discover.results || []).slice(0, count);
